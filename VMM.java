...
public class VMM extends Thread {
    ...
    public static void main (String args[]) throws FileNotFoundException{
		...
		//configure main memory into an array
		...
		//create new vm.txt file for virtual memory
		//File f = new File("./vm.txt");// variable id | variable value | age
		
		//read form command file
		...
		for(String input: commandFile){
			...
			switch (commands[0].toLowerCase()) {
				case "store": 
					System.out.println(v.memStore(Integer.parseInt(commands[1]),Integer.parseInt(commands[2])));
					break;
				case "release": 
					v.memFree(Integer.parseInt(commands[1]));
					break;
				case "lookup": 
					v.memLookup(Integer.parseInt(commands[1]));
					break;
